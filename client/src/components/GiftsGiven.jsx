...
export default function GiftsGiven({ token, gifts, onGiftAdded, onGiftUpdated, onGiftDeleted }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = modalMode === 'add' 
        ? 'http://localhost:5000/api/gifts/given'
        : `http://localhost:5000/api/gifts/given/${selectedGiftId}`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/gifts/given/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const totalAmountGiven = filteredGifts
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h2>My Gifts (Gifts Given)</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Keep track of what gifts and amounts you have presented to others.</p>
        </div>
        <button className="btn btn-primary" onClick={openAddModal}>
          <i className="fa fa-plus"></i> Record Gift Given
        </button>
      </div>
      ...
      <div className="dashboard-grid" style={{ marginBottom: '25px', gridTemplateColumns: '1fr 1fr' }}>
        <div className="metric-card">
          <div className="metric-header">
            <span>Total Cash / Amount Given</span>
        ...
        <div className="metric-card">
          <div className="metric-header">
            <span>Total Physical Gifts Given</span>
            <i className="fa fa-gift" style={{ color: 'var(--primary)' }}></i>
          </div>
          <div className="metric-value">{physicalGiftsCount}</div>
          <div className="metric-footer">Items such as dress, watch, household wares</div>
        </div>
      </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h3>{modalMode === 'add' ? 'Record Gift Given' : 'Edit Gift Given'}</h3>
              ...
              <div className="modal-body">
                ...
                {formData.gift_type === 'Amount' ? (
                  ...
                  <div className="form-group">
                    ...
                    <input 
                      ...
                      onChange={handleInputChange} 
                      className="form-control" 
                      placeholder="e.g. Fossil Leather Watch" 
                    />
                  </div>
                )}
                ...
                <div className="form-group">
                  ...
                  <textarea 
                    ...
                    onChange={handleInputChange} 
                    className="form-control" 
                    placeholder="Relationship details or memory notes..." 
                  />
                </div>
              </div>
